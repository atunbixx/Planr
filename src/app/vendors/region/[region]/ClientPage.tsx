...
export default function ClientRegionVendorsPage({ region, initial }: { region: string; initial: { vendors: DirVendor[]; total: number; pageSize: number } }) {
  ...
  useEffect(() => { load() }, [page, pageSize, region])
  useEffect(() => { setPage(1); load() }, [category, minRating, sort])
  ...
  const activeFilters = useMemo(() => {
    ...
    return arr
  }, [category, minRating, sort, q])

  return (
    <Box sx={{ px: { xs: 2, md: 4 }, py: 4 }}>
      <Head>
        <link rel="canonical" href={`${process.env.NEXT_PUBLIC_BASE_URL || ''}/vendors/region/${encodeURIComponent(region)}`} />
      </Head>
      <Script id="schema-vendor-region-list" type="application/ld+json">
      ...
      <Typography variant="h3" sx={{ fontFamily: '"Bodoni Moda", serif', fontWeight: 400, mb: 1.5 }}>Vendors in {region}</Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Browse vendors by region. Filter by category, rating, or search.</Typography>
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 1.5, alignItems: 'center' }}>
        ...
        <TextField select size="small" label="Category" value={category} onChange={(e) => { setCategory(e.target.value); setPage(1); }} sx={{ minWidth: { xs: 160, sm: 200 } }}>
          <MenuItem value="">All</MenuItem>
          {['venue','photographer','videographer','catering','florist','music'].map(c => <MenuItem key={c} value={c}>{c.charAt(0).toUpperCase()+c.slice(1)}</MenuItem>)}
        </TextField>
        ...
        {activeFilters.length > 0 && (
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1 }}>
              ...
              <Button size="small" onClick={() => { setCategory(''); setMinRating(''); setSort('newest'); setQ(''); setPage(1); load() }}>Clear All</Button>
            </Box>
          </Grid>
        )}
